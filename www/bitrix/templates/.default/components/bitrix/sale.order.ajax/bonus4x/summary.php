			...
			<table class="bx_ordercart_order_sum">
				<tbody>
					...
					if($arResult['MINUS']){?>
						...
						if (strlen($arResult["PAYED_FROM_BONUS_FORMATED"]) > 0)
						{
							...
							<tr>
								...
								<td class="custom_t2" class="price"><?=$arResult['BONUSPAY']["PAYED_FROM_BONUS_FORMATED"]?></td>
							</tr>
						<?}?>
						<tr>
							<td class="custom_t1 fwb" colspan="<?=$colspan?>" class="itog"><?=GetMessage("SOA_TEMPL_SUM_LEFT_TO_PAY")?></td>
							<td class="custom_t2 fwb" class="price"><?=$arResult['JS_DATA']['TOTAL']['ORDER_TOTAL_PRICE_FORMATED']?></td>
						</tr>
					...
					<?
						...
						<tr>
							<td class="custom_t1 fwb" colspan="<?=$colspan?>" class="itog">Бонус за заказ:</td>
							<td class="custom_t2 fwb" class="price">
							<?$APPLICATION->IncludeComponent(
								"vbcherepanov:vbcherepanov.bonusfororder","cart",
								Array(
									"TYPE" => "ORDER",
									"RESULT" => $arResult,
									"CACHE_TYPE" => "N",
