...
class RefcodeBonusAccount extends \CBitrixComponent
{
	/**
	...
	protected function getResult()
	{
		...
		if($option1['OPTION']=='Y'){
			$refcode=\ITRound\Vbchbbonus\CVbchRefTable::getList(array(
				'filter'=>array('ACTIVE'=>"Y",'USERID'=>$UID,'LID'=>SITE_ID),
				'select'=>array('REFERER'),
			))->fetch();
			if($refcode){
				$this->arResult=$refcode;
				$opt=$bb->GetOptions(SITE_ID,'REFPARAM');
				$opt=$opt['OPTION'];
				$request = Context::getCurrent()->getRequest();
				$url='';
				$url.=($request->isHttps()?'https':'http').'://';
				$url.=$request->getHttpHost();
				if(!$this->arParams['ONLY_MAIN']){
                    $url.=$request->getRequestUri();
                }
                $URI=new \Bitrix\Main\Web\Uri($url);
				$tmp=$URI->addParams(array($opt=>$refcode['REFERER']));
				$this->arResult['EXAMPLES']= $tmp->getUri();
			}else{
