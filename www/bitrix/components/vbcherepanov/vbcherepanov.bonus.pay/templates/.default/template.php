...
else
{
	if ($arParams['REFRESHED_COMPONENT_MODE'] === 'Y')
	{
                    ...
                    <div class="row">
                        <div class="col-xs-12 sale-acountpay-block form-horizontal">
                                <div class="form-group" style="margin-bottom: 0;">
                                    <div class='col-sm-9'>
                                        <label>
											<input type='checkbox' name="USE_COUPONE" class="use_coupone" value='N'>
                                            <?=Loc::getMessage("VBCHBB_USE_COUPONE")?>
                                        </label>
									</div>
                                </div>
                        </div>
		...
		$javascriptParams = array(
			...
			"templateName" => $this->__component->getTemplate()->GetName(),
		...
		?>
		<script>
			var sc = new BX.BonusPay(<?=$javascriptParams?>);
		</script>
	<?
	}
	else
	{
		...
		<form method="post" name="buyBonusForm" action="">
			<?
