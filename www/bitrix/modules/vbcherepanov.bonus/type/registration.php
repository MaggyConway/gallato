...
$profiles=array(
    "REGISTRATION",
    Loc::getMessage('VBCHBB_TYPE_PROFILE_REGISTRATION_NAME'),
    Loc::getMessage('VBCHBB_TYPE_PROFILE_REGISTRATION_DSC'),
    array(
        'TAB1'=>array(
    ...
    array(
        array(
            'ACTIVE'=>"N",
            'BITRIX'=>'main_OnAfterUserAdd',
            'MODULE'=>'CVbchbbEvents::OnRegUser',
            'RULES'=>'true;',
        ),
    ),
    array(),
    'GetRules'=>function ($profileID,$Filter=array(),$arFields=array()) {
        $lp=array();
        $flds['ACTIVATE']=$arFields['ACTIVE'];
        $flds['USERGROUP']=$arFields['GROUP_ID'];
        if($Filter['ONLYAUTH']=='Y'){
            $l=array_search(2,$arFields['GROUP_ID']);
            unset($arFields['GROUP_ID'][$l]);
        }
        unset($Filter['ONLYAUTH']);
        if(is_array($arFields['GROUP_ID']))
            $lp=current($arFields['GROUP_ID']);
        $gi=array();
        if(is_array($lp) && array_key_exists("GROUP_ID",$lp)){
            foreach($arFields['GROUP_ID'] as $idg){
                $gi[]=$idg['GROUP_ID'];
            }
            $arFields['GROUP_ID']=$gi;
        }
        return $this->GetFilterString($Filter,$flds);
    },
