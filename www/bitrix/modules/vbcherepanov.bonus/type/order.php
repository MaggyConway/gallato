...
$profiles = array(
    "ORDER",
    Loc::getMessage('VBCHBB_TYPE_PROFILE_ORDER_NAME'),
    Loc::getMessage('VBCHBB_TYPE_PROFILE_ORDER_DSC'),
    array(
        'TAB1' => array(
            ...
            "ELEM" => array(
                'SETTINGS' => array(
                    'ELEMENT' => array(
                        ...
                        'NONBONUSPAYBONUS' => array(
                            'WIDGET' => new ITRound\Vbchbbonus\CheckboxWidget(),
                            "TITLE" => Loc::getMessage('VBCHBB_TYPE_PROFILE_ELEMENT_NONBONUSPAYBONUS'),
                            'DEFAULT' => 'N',
                            "REQUIRED" => false,
                            "TYPE" => "string",
                            "MULTIPLE" => false,
    ...
    array(
        ...
        array(
            'ACTIVE' => "N",
            'BITRIX' => 'sale_OnSaleStatusOrder',
            'MODULE' => 'CVbchbbEvents::StatusOrderEvent',
            'RULES' => '$this->isD7();',
        ),
    ),
    array(),
    'GetRules' => function ($profileID, $Filter = array(), $arFields = array()) {
        ...
        if (!array_key_exists("BASKET", $arFields) || $arFields['BASKET'] == 1) {
        ...
        if ($arFields['DELIVERY_ID']) {
            ...
            $flds['ORDERDELIVERY'] = is_array($arFields['DELIVERY_ID']) ? (sizeof($arFields['DELIVERY_ID']) == 1 ? current($arFields['DELIVERY_ID']) : $arFields['DELIVERY_ID']) : intval($arFields['DELIVERY_ID']);
        }
        if ($arFields['PAY_SYSTEM_ID']) {
            $fltr['ORDERPAYMENT'] = $Filter['ORDERPAYMENT'];
            $flds['ORDERPAYMENT'] = is_array($arFields['PAY_SYSTEM_ID']) ? (sizeof($arFields['PAY_SYSTEM_ID']) == 1 ? current($arFields['PAY_SYSTEM_ID']) : $arFields['PAY_SYSTEM_ID']) : intval($arFields['PAY_SYSTEM_ID']);
        }
        ...
        if ($glFilter != '((1 == 1))') {
            ...
            $fltr['ELEMENTFILTER'] = 'N';
            $flds['ELEMENTFILTER'] = "N";
        }

        if ($Filter['NONEFIRSTORDER'] == 'Y') {
            $fltr['NONEFIRSTORDER'] = $Filter['NONEFIRSTORDER'];
            $flds['NONEFIRSTORDER'] = $this->isFirstOrder($arFields['USERID'], $arFields['ORDER_ID']) ? 'N' : 'Y';
        }
        ...
        if ($Filter['MINBONUSPAY1'] && is_array($Filter['MINBONUSPAY1']) && sizeof($Filter['MINBONUSPAY1']) > 0) {
            foreach ($Filter['MINBONUSPAY1'] as $p => $ll) {
                ...
                $flds['MINBONUSPAY1_' . $p] = ($l <= $Filter['MINBONUSPAY1'][$p][0] && $l <= $Filter['MINBONUSPAY1'][$p][1]) ? 'Y' : 'N';
            }
        }
    ...
    'GetBonus' => function ($profile = array(), $arFields = array()) {
        $bonus = 0;
        $innerbonus = false;
        $pay_order_bonus = $this->GetPayedBonusOrder($arFields['ORDER_ID']);
        $bonus = $profile['BONUS'];
        if ($arFields['ORDER_PRICE'] == 0 || $arFields['ORDER_PRICE'] == '') {
            foreach ($arFields['BASKET'] as $bask) {
        ...
        if ($glFilter != '((1 == 1))') {
            foreach ($arFields['BASKET'] as $idf => $arItems) {
                if (!eval('return ' . $glFilter . ';')) {
                    unset($arFields['BASKET'][$idf]);
                    $arFields['ORDER_PRICE'] = $arFields['ORDER_PRICE'] - ($arItems['PRICE'] * $arItems['QUANTITY']);
                }
            ...
            foreach ($arFields['BASKET'] as $idof => $arItem) {
                ...
                if ($this->CheckArray($arItem['DISCOUNT'])) {
                    ...
                    if ($del || $offer_price < $Settings['MINOFFERPRICE']) {
                        ...
                        $arFields['ORDER_PRICE'] = $arFields['ORDER_PRICE'] - ($arItem['PRICE'] * $arItem['QUANTITY']);
                    }
                }
        ...
        if (isset($arFields['ORDER_PRICE'])) {
            ...
            $ORDER_PRICE = $arFields['ORDER_PRICE'];
        }
        if ($pay_order_bonus != 0 || $pay_order_bonus != '') {
            $ORDER_PRICE = $ORDER_PRICE + $pay_order_bonus;
        }
        if ($Settings['WITHOUTPAYBONUS'] == 'Y') {
        ...
        if ($Settings['NONBONUSPAYBONUS'] == 'Y' && $arFields['BONUSPAY'] > 0) {
            $bonus = 0;
        }
