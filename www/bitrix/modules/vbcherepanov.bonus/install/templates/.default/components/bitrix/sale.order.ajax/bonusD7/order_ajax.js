...
(function() {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function(parameters)
        {
            ...
            this.bb_inner_pay=parameters.bb_inner_pay || false;
            this.bb_bonus_pay=parameters.bb_bonus_pay || false;
            this.bb_bonus_topay=parameters.bb_bonus_topay || false;
        ...
        editAuthorizeForm: function(authContent)
        {
            ...
            login = this.createAuthFormInputContainer(
                ...
                BX.create('INPUT', {
                    attrs: {'data-next': 'USER_PASSWORD','id':'USER_LOGIN'},
                    props: {
                        name: 'USER_LOGIN',
                        type: 'text',
        ...
        editPaySystemInfo: function(paySystemNode)
        {
            ...
            var paySystemInfoContainer = BX.create('DIV', {
                ...
                innerPs, extPs, delimiter, currentPaySystem,bonusPs,bonustopay,
                logotype, logoNode, subTitle, label, title, price;
            if(this.result.BONUSPAY){
                this.bb_bonus_pay=this.result.BONUSPAY;
            }else{
                this.bb_bonus_pay.DISPLAY=false;
            }
            if(this.result.SYSTEMPAY) {
                this.bb_inner_pay = this.result.SYSTEMPAY;
            }else{
                this.bb_inner_pay.DISPLAY=false;
            }
            BX.cleanNode(paySystemInfoContainer);

            if(this.params.SHOW_BONUS_SYSTEM=='Y' && this.result.IS_AUTHORIZED){
                if(this.bb_inner_pay.DISPLAY){
                    innerPs = this.getInnerPaySystem(paySystemInfoContainer);
                }
                if(this.bb_bonus_pay.DISPLAY) {
                    bonusPs = this.getBonusPaySystem(paySystemInfoContainer);
                }
                if(this.bb_bonus_topay.DISPLAY && (this.bb_inner_pay.BONUSORDERPAY || this.bb_bonus_pay.BONUSORDERPAY)) {
                    bonustopay = this.getBonusToPaySystem(paySystemInfoContainer);
                }
            }else{
                if (this.result.PAY_FROM_ACCOUNT !== 'N') {
                    innerPs = this.getInnerPaySystem(paySystemInfoContainer);
                }
            }
            ...
            paySystemInfoContainer.appendChild(
                BX.create('DIV', {
                    ...
                    children: [bonustopay,innerPs,bonusPs, delimiter, extPs]
                })
            );
        ...
        getInnerPaySystem: function()
        {
            if (!this.result.CURRENT_BUDGET_FORMATED || !this.result.PAY_CURRENT_ACCOUNT || !this.result.INNER_PAY_SYSTEM)
                return;
            var isSelected = this.result.PAY_CURRENT_ACCOUNT, paySystem = this.bb_inner_pay;
            ...
            if(this.result.SYSTEMPAY)
                payInputH = BX.create('INPUT', {
                    props: {
                        type: 'hidden',
                        id: 'ACCOUNT_CNT',
                        name: 'ACCOUNT_CNT',
                        value: this.result.SYSTEMPAY.MAXPAY ? this.result.SYSTEMPAY.MAXPAY : 0,

                    }
                });

            if(this.bb_inner_pay.USER_INPUT && this.result.SYSTEMPAY){
                payInput = BX.create('INPUT', {
                    props: {
                        type: 'text',
                        id: 'ACCOUNT_CNT',
                        name: 'ACCOUNT_CNT',
                ...
                htmlString = BX.util.htmlspecialchars(this.params.MESS_INNER_PS_BALANCE) + ' <b class="wsnw">' + this.bb_inner_pay.CURRENT_BUDGET_FORMATED
                    + '</b><br />';
                htmlString=htmlString+BX.util.htmlspecialchars(this.params.MESS_PAY_TO)+this.bb_inner_pay.ORDER_PAY_PERCENT;
            }else{
                htmlString = BX.util.htmlspecialchars(this.params.MESS_INNER_PS_BALANCE) + ' <b class="wsnw">' + this.bb_inner_pay.CURRENT_BUDGET_FORMATED
                    + '</b><br />';
                if(this.result.SYSTEMPAY)
                    htmlString=htmlString+BX.util.htmlspecialchars(this.params.MESS_PAY_TO1)+(this.result.SYSTEMPAY.MAXPAY ? this.result.SYSTEMPAY.MAXPAY :0 );
            }
            ...
            return BX.create('DIV', {
                ...
                children: [hiddenInput,subTitle, label, title, innerPsDesc,payInputH,payInput ]
            });
        },

        getBonusPaySystem: function()
        {
            var isSelected = true,
                payBonSystem = this.bb_bonus_pay,
                isSelected = this.result.PAY_BONUS_ACCOUNT,
                logotype, logoNode,subTitle, label, title, hiddenInput, htmlString, innerPsDesc,payInputH,payInput;

            if (this.params.SHOW_PAY_SYSTEM_INFO_NAME == 'Y')
            {
                subTitle = BX.create('DIV', {
                    props: {className: 'bx-soa-pp-company-subTitle'},
                    text: payBonSystem.NAME
                });
            }

            var help;
            if(this.params.ITR_BONUS_PATH_TO_HELP!='' && this.params.ITR_BONUS_TEXT_TO_HELP!='')
                help = BX.create('A',{props:{href:BX.util.htmlspecialchars(this.params.ITR_BONUS_PATH_TO_HELP)},html:BX.util.htmlspecialchars(this.params.ITR_BONUS_TEXT_TO_HELP)});

            logoNode = BX.create('DIV', {props: {className: 'bx-soa-pp-company-image'}});
            logotype = this.getImageSources(payBonSystem, 'LOGOTIP');
            if (logotype && logotype.src_2x)
            {
                logoNode.setAttribute('style',
                    'background-image: url(' + logotype.src_1x + ');' +
            ...
            if (payBonSystem.DESCRIPTION && payBonSystem.DESCRIPTION.length)
            {
                title = BX.create('DIV', {
                    props: {className: 'bx-soa-pp-company-block'},
                    children: [BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc'}, html: payBonSystem.DESCRIPTION})]
                });
            }
            if(payBonSystem.DISPLAY) {
                hiddenInput = BX.create('INPUT', {
                    props: {
                        type: 'hidden',
                        name: 'PAY_BONUS_ACCOUNT',
                ...
                payInputH = BX.create('INPUT', {
                    props: {
                        ...
                        id: 'BONUS_CNT',
                        name: 'BONUS_CNT',
                        value: payBonSystem.MAXPAY ? payBonSystem.MAXPAY : 0
                    }
                });
            }
            if(this.bb_bonus_pay.USER_INPUT){
                payInput = BX.create('INPUT', {
                    props: {
                        type: 'text',
                        id: 'BONUS_CNT',
                ...
                htmlString = BX.util.htmlspecialchars(this.params.MESS_BONUS_PS_BALANCE)+'<b class="wsnw">' + payBonSystem.CURRENT_BONUS_BUDGET_FORMATED
                    + '</b><br />';
                htmlString=htmlString+BX.util.htmlspecialchars(this.params.MESS_PAY_TO)+payBonSystem.ORDER_PAY_PERCENT;
            }
            else{
                htmlString = BX.util.htmlspecialchars(this.params.MESS_BONUS_PS_BALANCE)+'<b class="wsnw">' + payBonSystem.CURRENT_BONUS_BUDGET_FORMATED
                    + '</b><br />';
                htmlString=htmlString+BX.util.htmlspecialchars(this.params.MESS_PAY_TO1)+(payBonSystem.MAXPAY ? payBonSystem.MAXPAY : 0);
            }

            innerPsDesc = BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc'}, html: htmlString});

            return BX.create('DIV', {
                props: {className: 'bx-soa-bonus-bonuspay-ps' + (isSelected ? ' bx-selected' : '')},
                children: [hiddenInput,subTitle, label, title, help,innerPsDesc,payInputH,payInput ]
            });
        },

        getBonusToPaySystem: function()
        {
            var isSelected = true,
                payToBonSystem = this.bb_bonus_topay,
                isSelected = this.result.PAY_BONUSORDERPAY,
                logotype, logoNode,subTitle, label, title, hiddenInput, htmlString, innerPsDesc;
            if (this.params.SHOW_PAY_SYSTEM_INFO_NAME == 'Y')
            {
                subTitle = BX.create('DIV', {
                    props: {className: 'bx-soa-pp-company-subTitle'},
                    text: payToBonSystem.NAME
                });
            }

            logoNode = BX.create('DIV', {props: {className: 'bx-soa-pp-company-image'}});
            logotype = this.getImageSources(payToBonSystem, 'LOGOTIP');
            if (logotype && logotype.src_2x)
            {
                logoNode.setAttribute('style',
                    'background-image: url(' + logotype.src_1x + ');' +
            ...
            label = BX.create('DIV', {
                ...
                children: [
                    BX.create('DIV', {
                        ...
                        events: {
                            click: BX.proxy(this.selectBonusToPaySystem, this)
                        }
                    })
                ]
            });

            if (payToBonSystem.DESCRIPTION && payToBonSystem.DESCRIPTION.length)
            {
                title = BX.create('DIV', {
                    props: {className: 'bx-soa-pp-company-block'},
                    children: [BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc'}, html: payToBonSystem.DESCRIPTION})]
                });
            }
            ...
            htmlString=BX.util.htmlspecialchars('');
            innerPsDesc = BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc'}, html: htmlString});

            return BX.create('DIV', {
                props: {className: 'bx-soa-bonus-bonustopay-ps' + (isSelected ? ' bx-selected' : '')},
        ...
        editFadePaySystemContent: function(node)
        {
            ...
            if(this.result.PAY_BONUS_ACCOUNT){
                logotype = this.getImageSources(this.result.INNER_BONUS.BONUSPAY, 'LOGOTIP');
                imgSrc = logotype && logotype.src_1x || this.defaultPaySystemLogo;

                addedHtml += '<div class="bx-soa-pp-company-selected">';
                addedHtml += '<img src="' + imgSrc + '" style="height:18px;width:37px;" alt="">';
                addedHtml += '<strong>' + this.result.INNER_BONUS.BONUSPAY.NAME + '</strong><br />';
                addedHtml += '</div>';
            }
        ...
        selectBonusPaySystem: function(event)
        {
            if (!this.orderBlockNode || !event)
                return;

            var target = event.target || event.srcElement,
                bonusPaySystemSection = this.paySystemBlockNode.querySelector('div.bx-soa-bonus-bonuspay-ps'),
                bonusPaySystemCheckbox = this.paySystemBlockNode.querySelector('input[type=checkbox][name=PAY_BONUS_ACCOUNT]'),
                fullPayFromBonusPaySystem = this.result.TOTAL && parseFloat(this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY) === 0;

            var bonusPsAction = BX.hasClass(target, 'bx-soa-bonus-bonuspay-ps') ? target : BX.findParent(target, {className: 'bx-soa-bonus-bonuspay-ps'}),
            ...
            if (bonusPsAction)
            {
                if (target.nodeName == 'INPUT')
                    bonusPaySystemCheckbox.checked = !bonusPaySystemCheckbox.checked;

                if (bonusPaySystemCheckbox.checked)
                {
                    BX.removeClass(bonusPaySystemSection, 'bx-selected');
                    bonusPaySystemCheckbox.checked = false;
                }
                else
                {
                    BX.addClass(bonusPaySystemSection, 'bx-selected');
                    bonusPaySystemCheckbox.checked = true;
                }
            }
            else if (actionSection)
            {
                ...
                if (bonusPaySystemCheckbox && bonusPaySystemCheckbox.checked && fullPayFromBonusPaySystem)
                {
            ...
            //BX.adjust(BX('BONUS_CNT'), {props: {disabled: bonusPaySystemCheckbox.checked}});
            this.sendRequest();
        },

        selectBonusToPaySystem: function(event)
        {
            if (!this.orderBlockNode || !event)
                return;

            var target = event.target || event.srcElement,
                bonusPaySystemSection = this.paySystemBlockNode.querySelector('div.bx-soa-bonus-bonustopay-ps'),
                bonusPaySystemCheckbox = this.paySystemBlockNode.querySelector('input[type=checkbox][name=PAY_BONUSORDERPAY]'),
                fullPayFromBonusPaySystem = this.result.TOTAL && parseFloat(this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY) === 0;

            var bonusPsAction = BX.hasClass(target, 'bx-soa-bonus-bonustopay-ps') ? target : BX.findParent(target, {className: 'bx-soa-bonus-bonustopay-ps'}),
                actionSection = BX.hasClass(target, 'bx-soa-pp-company') ? target : BX.findParent(target, {className: 'bx-soa-pp-company'}),
                actionInput, selectedSection;
        ...
        editTotalBlock: function()
        {
            ...
            if(this.params.SHOW_BONUS_SYSTEM=='Y'){
                if(total.BONUS_PAY && total.BONUS_PAY!=0){
                    this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('VBCH_BB_BONUS_PAY'), total.BONUS_PAY));
                }
                if(total.ACCOUNT_PAY && total.ACCOUNT_PAY!=0){
                    this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('VBCH_BB_ACCOUNT_PAY'), total.ACCOUNT_PAY));
                }
                if(total.BONUS_ORDER && total.BONUS_ORDER!=0){
                    this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('VBCH_BB_BONUS_ORDER'), total.BONUS_ORDER));
                }
            }
